...
bool ByteArray::isLittleEndian() const {
	return m_endian == ABOO_LITTLE_ENDIAN;
}

void ByteArray::setIsLittleEndian(bool val) {
	if (val) {
		m_endian = ABOO_LITTLE_ENDIAN;
	} else {
		m_endian = ABOO_BIG_ENDIAN;
	}
}
...
bool ByteArray::writeToFile(const std::string& name) const {
	...
	if (!ofs) {
		ABOO_LOG_ERROR(g_logger) <<  "writeToFile name=" << name << " error, errno=" << errno << " strerr=" << strerror(errno);
		return false;
	}
...
bool ByteArray::readFromFile(const std::string& name) {
	...
	if (!ifs) {
		ABOO_LOG_ERROR(g_logger) << "readFromFile name=" << name << " error, errno=" << errno << " strerr=" << strerror(errno);
		return false;
	}
