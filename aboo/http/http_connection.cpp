...
HttpConnection::~HttpConnection() {
	ABOO_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
	...
	if (client_parser.chunked) {
		...
		do {
			...
			ABOO_LOG_DEBUG(g_logger) << "content_len=" << client_parser.content_len;
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req
	...
	if (!sock) {
		return std::make_shared<HttpResult>((int)HttpResult::Error::CREATE_SOCKET_ERROR
				...
				+ " strerr=" + std::string(strerror(errno)));
	}
	...
	if (rt < 0) {
		return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_SOCKET_ERROR
				, nullptr, "send request socket error errno= " + std::to_string(errno) + " strerr=" + std::string(strerror(errno)));
	}
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
	uint64_t now_ms = aboo::GetCurrentMS();
	...
	if (!ptr) {
		...
		if (!addr) {
			ABOO_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
			return nullptr;
		}
		addr->setPort(m_port);
		Socket::ptr sock = Socket::CreateTCP(addr);
		if (!sock) {
			ABOO_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
			return nullptr;
		}
		if (!sock->connect(addr)) {
			ABOO_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
			return nullptr;
		}
...
HttpResult::ptr HttpConnectionPool::doRequest(HttpRequest::ptr req
	...
	if (!sock) {
		return std::make_shared<HttpResult>((int)HttpResult::Error::POOL_INVALID_CONNECTION
				, nullptr, "invalid socket connection");
	}
	sock->setRecvTimeout(timeout_ms);
	int rt = conn->sendRequest(req);
	if (rt == 0) {
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
	++ ptr->m_request;
	if (!ptr->isConnected() || ((ptr->m_createTime + pool->m_maxAliveTime) >= aboo::GetCurrentMS()) || ((int32_t)ptr->m_request >= pool->m_maxRequest)) {
		delete ptr;
		ptr = nullptr;
		-- pool->m_total;
		return;
