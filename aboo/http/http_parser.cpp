...
static aboo::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
	aboo::Config::Lookup("http.request.buffer_size",
				(uint64_t)(4 * 1024), "http request buffer size");

static aboo::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
...
void on_request_method(void *data, const char *at, size_t length) {
	...
	if (m == HttpMethod::INVALID_METHOD) {
		ABOO_LOG_WARN(g_logger) << "invalid	http request method "
...
void on_request_version(void *data, const char *at, size_t length) {
	...
	if (strncmp(at, "HTTP/1.1", length) == 0) {
		...
		ABOO_LOG_WARN(g_logger) << "invalid http request version: " << std::string(at, length);
...
HttpRequestParser::HttpRequestParser()
	: m_error(0) {
	m_data.reset(new aboo::http::HttpRequest);
...
void on_response_version(void *data, const char *at, size_t length) {
	...
	if (strncmp(at, "HTTP/1.1", length) == 0) {
		...
		ABOO_LOG_WARN(g_logger) << "invalid http response version: " << std::string(at, length);
		parser->setError(1001);
		return;
	}
	parser->getData()->setVersion(v);
...
HttpResponseParser::HttpResponseParser()
	: m_error(0) {
	m_data.reset(new aboo::http::HttpResponse);
