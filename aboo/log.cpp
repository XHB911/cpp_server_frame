...
void Logger::setFormatter(std::string val) {
	aboo::LogFormatter::ptr new_val(new aboo::LogFormatter(val));
	if (new_val->isError()) {
...
#define XX(str, C) \
		{#str, [](const std::string& fmt)->FormatItem::ptr { return FormatItem::ptr(new C(fmt)); } }
		...
		XX(T, TabFormatItem),		// Tab键
		XX(F, FiberIdFormatItem),		// Tab键
#undef XX
	};
	...
	//std::cout << "m_items num is " << m_items.size() << std::endl;
}
...
aboo::ConfigVar<std::set<LogDefine> >::ptr g_log_defines = aboo::Config::Lookup("logs", std::set<LogDefine>(), "logs config");

struct LogIniter {
	LogIniter() {
		g_log_defines->addListener([](const std::set<LogDefine>& old_value, const std::set<LogDefine>& new_value){
			ABOO_LOG_INFO(ABOO_LOG_ROOT()) << "on_logger_conf_changed";
			for (auto i : new_value) {
				auto it = old_value.find(i);
				aboo::Logger::ptr logger;
				if (it == old_value.end()) {
					// 新增的logger
					logger = ABOO_LOG_NAME(i.name);
				} else {
					if (!(i == *it)) {
						// 修改的logger
						logger = ABOO_LOG_NAME(i.name);
			...
			for (auto i : old_value) {
				...
				if (it == new_value.end()) {
					// 删除log
					auto logger = ABOO_LOG_NAME(i.name);
